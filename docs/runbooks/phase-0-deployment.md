...
5. Run the authenticated staging smoke test. Confirm account creation, token refresh, invalid-token rejection, submission behavior for the current poll state, deletion receipt idempotency, and aggregate correction. This immediate smoke test proves that cleanup was scheduled; it does not prove eventual anonymous-account deletion.
6. After the one-hour deletion delay and the next hourly cleanup execution, separately verify the cleanup scheduler succeeded, the deletion request no longer exists, and the test account can no longer refresh or authenticate. Record scheduler, Firestore, and Authentication evidence in the cutover record. Do not treat the immediate smoke artifact as delayed-cleanup completion evidence.
7. Deploy anonymous authentication to production through the independently approved `auth` component. Confirm a test account can be created, refreshed, revoked, and removed before store publication.
8. Deploy the response identity-hash collection-group index and wait until Firebase reports it ready:

   ```bash
   firebase deploy --only firestore:indexes --project production
   ```

9. Deploy only Functions after the index is ready:
...
12. Deploy temporary legacy rules only after the export and API smoke test succeed. Provide the immutable backup URI and SHA-256 to the deployment workflow so its preflight artifact binds the rules mutation to the approved backup:

   ```bash
   firebase deploy --only firestore:rules --project production
   ```

13. Stop at the Chrome Web Store publication gate. Review and approve the complete public presentation package: listing title and summary, detailed description, privacy disclosures, category, support and privacy URLs, screenshots, promotional tiles, icon rendering, and release notes.
14. Publish extension `1.0.5` only after that approval. Record Chrome Web Store submission, approval, publication, and observed adoption timestamps.
15. Monitor API errors, auth failures, duplicate rate, aggregate consistency, deletion-queue age, provider sync, spend, and support reports. Do not infer migration completion from passive analytics consent.
16. Keep the temporary rules for at least 30 days from confirmed publication and use store-version adoption evidence plus support risk review before final cutover.
17. Deploy final rules with the separate immutable config and the same independently verified backup evidence:

   ```bash
   firebase --config firebase.final.json deploy --only firestore:rules --project production
   ```

18. Run a packaged legacy-client check: reads still work, direct writes fail, and the UI shows sync unavailable rather than a false success.
...
Record commit SHA, extension version, Functions revisions, rule file SHA-256, immutable export URI, export SHA-256, record count, Firebase project ID, regions, operators, timestamps, immediate smoke-test results, delayed-cleanup evidence, rollback package hash, grace-period start/end, and final approval.
